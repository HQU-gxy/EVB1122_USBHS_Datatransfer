...
void *System_GetPDCaliParaAddr(void) { return (void *)&PD_CalibrationPara; }

uint32_t System_GetPDCaliParaLen(void) { return sizeof(PD_CalibrationPara); }
...
void CmdProc_NopConfig(void) {
	if (enableNopConfig && CmdModeFlag == 0) {
		...
		for (loop = 0; loop < nopConfigStartRegisterIndex; loop++) {
			...
			// printf("CmdProc_NopConfig chip0: g_ChannelCount = %d  addr=0x%02x
...
@返回：index，待发送数据长度
*********************************************/
uint16_t FillCmdAck(uint16_t *data, uint16_t dataLen, uint16_t cmdType,
					uint16_t status) {
	uint16_t index = 0;
	...
	if (data == NULL) {
		...
		*((uint16_t *)(&CmdAck[index])) =
			CMD_TYPE_LEN + CMD_STATUS_LEN + (dataLen * sizeof(uint16_t));
	} else {
		*((uint16_t *)(&CmdAck[index])) =
			CMD_TYPE_LEN + (dataLen * sizeof(uint16_t));
	}
...
@返回：pos，待发送数据长度
*********************************************/
static uint16_t IAP_FillCmdACK(uint16_t CmdType, uint8_t *pData,
							   uint16_t dataLen, uint16_t ACK_Status) {
	uint16_t pos = 0;
	uint16_t len = 4;
	uint32_t i   = 0;
...
/********************************************
 ...
 @返回：ackLen，待发送数据长度
*********************************************/
static uint16_t FillReadSnCmdAck(uint16_t *data, uint16_t dataLen,
								 uint16_t cmdType, uint16_t status) {
	uint16_t index = 0;
	...
	// data Len
	*((uint16_t *)(&CmdAck[index])) =
		CMD_TYPE_LEN + CMD_STATUS_LEN + CMD_STR_LEN + dataLen;
	index += sizeof(uint16_t);
...
static uint16_t DoWriteReg(CMD_T *cmd, uint32_t cmdLen) {
	...
	if (CmdModeFlag) {
		...
		for (loop = 0; loop < regNum; loop++) {
			cmdReg = (CMD_REG_T *)((uint8_t *)(cmd->cmdData) + CMD_DEV_ADDR_LEN +
								   (loop * sizeof(CMD_REG_T)));
			SPI4_Write(devAddress, (uint8_t)(cmdReg->addr), cmdReg->val);
			if (devAddress == I2C_ADDR_BanYan_Chip0) { // update master register
...
static uint16_t DoReadReg(CMD_T *cmd, uint32_t cmdLen) {
	...
	if (CmdModeFlag) {
		...
		if (regNum > CMD_REG_MAX) {
			...
			uint16_t dataIndex =
				(masterProtocolVer != 0) ? CMD_DATA_POS : CMD_DATA_POS_OLD;
...
__weak int8_t MTT_ParaUpdate(uint16_t type, int32_t val) { return -1; }

__weak int8_t fftzerofill_ParaUpdate(uint16_t type, int32_t val) { return -1; }
...
static uint16_t DoWritePara(CMD_T *cmd, uint32_t cmdLen) {
	...
	if (CmdModeFlag) {
		...
		for (loop = 0; loop < paraNum; loop++) {
			cmdPara = (CMD_PARA_T *)((uint8_t *)(cmd->cmdData) +
									 (loop * sizeof(CMD_PARA_T)));
			switch (cmd->cmdType) {
			case WRITE_MTT_CMD:
...
//                    }
			...
			case FFT_ZEROFILL_CMD:
				...
				break;
			//                case NOP_CONFIG_CMD:
			//                    ret = NopCofig_ParaUpdate(cmdPara->type,
			//                    cmdPara->val); break;
			default:
				ret = -1;
				break;
	...
	if (sysPara.useMcuPackWrapper) {
		...
		if (CmdModeFlag) {
			...
			ackLen = 0; // For MCU_SPECIAL_FUNC_CMD, don't need to give the response
						// in non command mode
		}
	} else {
		ackLen = FillCmdAck(NULL, 0, cmd->cmdType, status);
...
static uint16_t DoGetAppRun(CMD_T *cmd, uint32_t cmdLen) {
	...
	return IAP_FillCmdACK(cmd->cmdType, (uint8_t *)&ack_data, sizeof(ack_data),
						  status);
}
	...
	if (WRITE_PD_CALIBRATION_CMD == cmd->cmdType) {
		...
		for (uint8_t i = 0; i < ((cmdLen - 2) / 6); i++) {
			if (0x0000 == cmd->cmdData[3 * i]) {
				...
				PD_CalibrationPara.calibrated_PhaseDiff =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0010 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib1_Temper =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0011 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib1_Power =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0012 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib1_Refval =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0013 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib1_SlopeVal =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0020 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib2_Temper =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0021 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib2_Power =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0022 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib2_Refval =
					(uint32_t)((highPart << 16) | lowPart);
			} else if (0x0023 == cmd->cmdData[3 * i]) {
				lowPart  = cmd->cmdData[3 * i + 1];
				highPart = cmd->cmdData[3 * i + 2];
				PD_CalibrationPara.PDCalib2_SlopeVal =
					(uint32_t)((highPart << 16) | lowPart);
			}
		}
	}
...
static void CmdExec(CMD_T *cmd, uint32_t cmdLen) {
	uint16_t ackLen       = 0;
	uint16_t devVersion[] = {RADAR_DEV_TYPE, RADAR_DEV_FIRMWARE_TYPE,
							 RADAR_DEV_VER_MAJAR, RADAR_DEV_VER_MINOR,
							 RADAR_DEV_VER_PATCH};
	...
	case START_CFG_CMD:
		...
		if (cmdLen > CMD_TYPE_LEN) {
			...
			ackLen            = FillCmdAck(boardInfomation, ARRAY_SIZE(boardInfomation),
										   cmd->cmdType, ACK_OK);
		} else {
			masterProtocolVer = 0;
	...
	case READ_VER_CMD:
		if (masterProtocolVer != 0) {
			ackLen =
				FillCmdAck(devVersion, ARRAY_SIZE(devVersion), cmd->cmdType, ACK_OK);
		} else {
...
*********************************************/
uint8_t CmdProc_InCmdMode(void) { return CmdModeFlag; }
...
/********************************************
 ...
 @返回：none
*********************************************/
void CmdProc_Init(void) { memset(&g_cmdRecv, 0, sizeof(g_cmdRecv)); }
...
*********************************************/
uint8_t CmdProc_IsInDebugMode(void) { return DebugModeFlag; }
