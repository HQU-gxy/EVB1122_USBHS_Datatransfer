...
static void DataCopy(uint8_t *buf, uint16_t len, uint8_t channel, uint16_t *i) {
	...
	copyLen = (RadarDataParse[channel].needCopyLen > (len - *i)) ? (len - *i) : RadarDataParse[channel].needCopyLen;
...
void DataProcTask(void const *argument) {
	...
	while (1) {
		for (loop = 0; loop < 2 * g_ChipCount; loop++) {
			if (DATA_TYPE_DFFT_PEAK == RadarPara.dataType || RadarPara.mergeData) {
				if (loop % 2 == 1) {
					continue;
